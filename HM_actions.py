...
# ------------------- START: NEW CHART FIG 2.2 ADDED HERE -------------------

# FIG 2.2: Reject Percentage by CEFR Score
# Group by DATE_GROUP and TALKSCORE_CEFR to get detailed rejection stats
df_rej_cefr = df_f.groupby(["DATE_GROUP", "TALKSCORE_CEFR"], as_index=False).agg(
   MOVED_BY_MANAGER=('MOVED_BY', 'count'),          # Total moves by manager for this group
   REJECTED_BY_MANAGER=('REJECTED_BY_MANAGER', 'sum') # Total rejections by manager for this group
)

# Calculate reject percentage for each CEFR group
df_rej_cefr["REJECT_PERCENT"] = (df_rej_cefr["REJECTED_BY_MANAGER"] / df_rej_cefr["MOVED_BY_MANAGER"]) * 100

# Create the multi-line chart for rejections
fig_rej_cefr = px.line(
   df_rej_cefr,
   x="DATE_GROUP",
   y="REJECT_PERCENT",
   color="TALKSCORE_CEFR",  # Use TALKSCORE_CEFR to create multiple lines
   markers=True,
   title="Reject % by CEFR Score Over Time",
   labels={"DATE_GROUP": "Time", "REJECT_PERCENT": "Rejection %", "TALKSCORE_CEFR": "CEFR Score"},
   line_shape="linear",
   category_orders={"TALKSCORE_CEFR": cefr_category_order}  # Re-use the same custom sort order
)

# Show the new chart
st.plotly_chart(fig_rej_cefr, use_container_width=True)

# -------------------- END: NEW CHART FIG 2.2 ADDED HERE --------------------
